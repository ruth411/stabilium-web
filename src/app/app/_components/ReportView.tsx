...
function ConversationMetrics({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
      <h3 className="mb-3 text-xs font-semibold uppercase" style={{ color: "#d4d4d4", letterSpacing: "0.05em" }}>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {Object.entries(averages).map(([key, value]) => (
          <div key={key} className="rounded-lg p-3" style={{ background: "rgba(255,255,255,0.04)" }}>
            <p className="mb-1 text-[11px] capitalize" style={{ color: "#d4d4d4" }}>{key.replaceAll("_", " ")}</p>
              ...
              >
                ...
                <div className="grid gap-2 sm:grid-cols-3">
                  {(["trajectory_consistency", "tool_selection_accuracy", "goal_completion_rate"] as const).map((metric) => {
                    ...
                    return (
                      <div key={metric}>
                        <div className="mb-1 flex items-center justify-between text-[11px]" style={{ color: "#d4d4d4" }}>
...
export function ReportView({ data }: ReportViewProps) {
    ...
    return (
      <div className="space-y-4">
        ...
        {completed != null && total != null && (
          <p className="text-sm" style={{ color: "#d4d4d4" }}>{completed} of {total} cases completed before failure.</p>
        )}
  ...
  const color = mean_asi != null ? asiColor(mean_asi) : "#d4d4d4";
  const radius = 54;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-6">
        ...
        <div className="flex flex-col items-center gap-3">
          <svg width="148" height="148" viewBox="0 0 148 148">
            ...
            <text x="74" y="86" textAnchor="middle" fill="#d4d4d4" fontSize="11" fontFamily="sans-serif">
              {scoreTitle}
            </text>
          </svg>
          {mean_asi != null && (
        ...
        <div className="flex flex-1 flex-col justify-center gap-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <p className="mb-1 text-xs" style={{ color: "#d4d4d4" }}>{label}</p>
          ...
          {asi_statistics && typeof asi_statistics.ci_low === "number" && typeof asi_statistics.ci_high === "number" && (
            <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <p className="mb-3 text-xs font-medium" style={{ color: "#d4d4d4" }}>95% confidence interval</p>
              ...
              {typeof asi_statistics.std === "number" && (
                <p className="mt-2 text-xs" style={{ color: "#d4d4d4" }}>
      ...
      {Object.keys(domain_scores).length > 0 && (
        <div>
          <h3 className="mb-4 text-xs font-semibold uppercase" style={{ color: "#d4d4d4", letterSpacing: "0.05em" }}>
            Domain breakdown
          </h3>
      ...
      <p className="border-t pt-4 text-xs" style={{ color: "#d4d4d4", borderColor: "rgba(255,255,255,0.06)" }}>
